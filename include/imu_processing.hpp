...
private:
  void IMUInit(const common::MeasureGroup &meas,
               esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N);
  void UndistortPcl(const common::MeasureGroup &meas,
                    esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
...
void ImuProcess::SetExtrinsic(const common::V3D &transl,
                              const common::M3D &rot) {
...
void ImuProcess::IMUInit(const common::MeasureGroup &meas,
                         esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
                         int &N) {
...
void ImuProcess::UndistortPcl(
    const common::MeasureGroup &meas,
    esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
    PointCloudType &pcl_out) {
  /*** add the imu_ of the last frame-tail to the of current frame-head ***/
  auto v_imu = meas.imu_;
  v_imu.push_front(last_imu_);
  const double &imu_beg_time = v_imu.front()->header.stamp.sec +
                               v_imu.front()->header.stamp.nanosec * 1e-9;
  const double &imu_end_time = v_imu.back()->header.stamp.sec +
                               v_imu.back()->header.stamp.nanosec * 1e-9;
  ...
  IMUpose_.clear();
  IMUpose_.push_back(common::set_pose6d(0.0, acc_s_last_, angvel_last_,
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
    ...
    //     continue;
    // }
    if (tail->header.stamp.sec + tail->header.stamp.nanosec * 1e-9 <
    ...
    // ROS2
    if (head->header.stamp.sec + head->header.stamp.nanosec * 1e-9 <
        last_lidar_end_time_) {
      dt = tail->header.stamp.sec + tail->header.stamp.nanosec * 1e-9 -
           last_lidar_end_time_;

    } else {
      dt = (tail->header.stamp.sec + tail->header.stamp.nanosec * 1e-9) -
           (head->header.stamp.sec + head->header.stamp.nanosec * 1e-9);
    }
    in.acc = acc_avr;
    in.gyro = angvel_avr;
    ...
    double &&offs_t = tail->header.stamp.sec +
                      tail->header.stamp.nanosec * 1e-9 - pcl_beg_time;
    IMUpose_.emplace_back(common::set_pose6d(offs_t, acc_s_last_, angvel_last_,
                                             imu_state.vel, imu_state.pos,
                                             imu_state.rot.toRotationMatrix()));
  }
  ...
  for (auto it_kp = IMUpose_.end() - 1; it_kp != IMUpose_.begin(); it_kp--) {
    ...
    for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
      ...
      common::V3D T_ei(pos_imu + vel_imu * dt + 0.5 * acc_imu * dt * dt -
                       imu_state.pos);
...
void ImuProcess::Process(const common::MeasureGroup &meas,
                         esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
                         PointCloudType::Ptr cur_pcl_un_) {
  if (meas.imu_.empty()) {
    return;
  ...
  Timer::Evaluate([&, this]() { UndistortPcl(meas, kf_state, *cur_pcl_un_); },
                  "Undistort Pcl");
}
} // namespace faster_lio
