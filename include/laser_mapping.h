...
public:
  ...
  // void PublishFrameEffectWorld(const ros::Publisher
  // &pub_laser_cloud_effect_world); void Savetrajectory(const std::string
  // &traj_file);

  // ROS2
  void PublishPath(rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pub_path);
  void PublishOdometry(rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr
                           pub_odom_aft_mapped);
  void PublishFrameWorld();
  void
  PublishFrameBody(rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr
...
private:
  ...
  template <typename T> void SetVelstamp(T &out);
